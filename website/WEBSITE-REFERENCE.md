...
- **2026-09-24**: Scott made the site live at the root domain,
  `checkpointplanning.com` (the `start.` subdomain plan was dropped).
  All destination URLs, docs, and campaign UTM examples across the repo
  updated to match. Two bugs found and fixed the same day: (1) lead
  forms were showing a success message without saving anything to the
  Google Sheet, `google-apps-script/Code.gs` hardened (explicit sheet
  lookup, a failure-alert email to Scott so a broken deployment can't
  silently vanish again) and `docs/DEPLOY.md` rewritten with a full
  redeploy/troubleshooting checklist, since the live Apps Script
  deployment itself needs Scott to redeploy it, that's not something
  fixable from this repo alone. (2) The lead-magnet forms promised
  "check your inbox," but no automation ever actually emailed the
  guide, so nothing was ever delivered. Replaced with an unlisted
  `resources.html` page (not in navigation, `noindex`), each form's
  success state now links straight to the visitor's guide. The three
  guides that only existed as markdown (early retirement, divorce,
  inheritance) are now also branded PDFs, generated by the new
...
- **Three new guide PDFs are functional, not fully designed**: built
  ...
  workbook's hand-designed layout. Worth a real design pass later if
  Scott wants them to match more closely.
